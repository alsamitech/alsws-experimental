...
int main(int argc, char** argv) {
	...
	for(;;){
		...
		fflush(stdout);
		char client_addr[128+1];
		connfd=accept(listenfd, (SA *) &addr, &addr_len);
		inet_ntop(AF_INET, &addr, client_addr,128);
		//fprintf(FN, "Client IP: %s\n", client_addr);
		fputs("Client IP:", stdout);
		fputs(client_addr, stdout);
		//printf("Client IP: %s\n", client_addr);
		SAYHI
